  ...
  try {
    ...
    // Каталог
      ...
      const page = data.startsWith('catalog_page_') 
        ...
        : 0;
      await showCatalog(bot, chatId, page);
    }
...
Пример: https://drive.google.com/file/...
        `, { parse_mode: 'Markdown' });
      }
    }
    ...
    // Подтверждение товара
      ...
      bot.sendMessage(chatId, '❌ Добавление товара отменено.');
    }
    ...
    // Мои заказы
      ...
      const user = await User.findOne({ telegram_id: telegramUser.id });
      bot.sendMessage(chatId, `💰 Ваш баланс: ${user.balance || 0} USDT`);
    }
      ...
      bot.sendMessage(chatId, '🔍 Введите поисковый запрос:');
      if (!global.userStates) global.userStates = {};
    ...
    bot.answerCallbackQuery(query.id, { text: '❌ Ошибка. Попробуйте позже.' });
  }
});
...
bot.on('message', async (msg) => {
  ...
  if (state.action === 'searching') {
    await searchProducts(bot, chatId, text);
    delete global.userStates[chatId];
    return;
  }
...
bot.onText(/\/catalog/, async (msg) => {
  const chatId = msg.chat.id;
  await showCatalog(bot, chatId, 0);
});
...
async function showMyOrders(bot, chatId, telegramUser) {
  try {
    ...
    if (lang === 'en') message = `📦 **My Orders**\n\n`;

    orders.forEach((order, index) => {
      const product = order.product_id;
      const status = t[order.status] || order.status;
      
      message += `${index + 1}. **${product?.title || 'Товар'}**\n`;
    ...
    bot.sendMessage(chatId, '❌ Произошла ошибка при загрузке заказов.');
  }
}
...
2. Нажмите "Купить"
3. Отправьте USDT на указанный адрес
4. Получите товар автоматически

**Как продавать:**
1. Используйте /sell
2. Добавьте свой товар
3. Укажите цену в USDT
4. Получайте деньги с каждой продажи
